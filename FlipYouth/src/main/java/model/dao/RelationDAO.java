...
public class RelationDAO {
	...
	public List<RelationBean> selectAllFriend(Integer mbrSN) {
		...
		Query query = this.getSession().createQuery("from RelationBean where mbrSN = " + mbrSN + "and relation = 1");

		// 這是SQL的語法 在createQuery中不能這樣寫------select * from relation where mbrSN =
		// '" + mbrSN + "' AND relation = '2'"
		// List<RelationBean> list =
		// (List<RelationBean>) this.getSession().createQuery("select * from
		// relation where mbrSN = '" + mbrSN + "' AND relation = '1'");
	...
	public List<RelationBean> selectAllBlockadeMember(Integer mbrSN) {
		...
		System.out.println("relationDAO mbrSN (使用者號碼) = " + mbrSN);
		Query query = this.getSession().createQuery("from RelationBean where mbrSN = " + mbrSN + "and relation = 2");
	...
	// ***************** 單獨搜尋一個好友
	// *******************************************************************
	public RelationBean selectOne(Integer mbrSN, Integer targetMbrSN) {
		System.out
				.println("RelationBean selectOne  Integer mbrSN= " + mbrSN + " ,Integer targetMbrSN = " + targetMbrSN);
		Query query = this.getSession()
				.createQuery("from RelationBean where targetMbrSN = " + targetMbrSN + "and mbrSN = " + mbrSN);
			
		if (query.getResultList().size() != 0) {
			return (RelationBean) query.getResultList().get(0);
		} else {
			return null;
		}
	}
	...
	public void deleteFriend(String mbrSN, int targetMbrSN) {
		...
		Query query = this.getSession().createQuery("delete  RelationBean where mbrSN =" + mbrSN + " and targetMbrSN ="
				+ targetMbrSN + " and relation = 1 ");
